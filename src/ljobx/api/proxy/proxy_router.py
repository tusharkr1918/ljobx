...
class ProxyRouter:
    ...
    def create_providers_from_config(cls, config_data: dict) -> List[ProxyProvider]:
        ...
        for config in provider_configs:
            ...
            logger.info(f"Initialized provider '{name}' with config: {provider_args}")
    ...
    async def _check_proxy(proxy: str, timeout: int = 5) -> str | None:
        ...
        try:
            ...
            logger.debug(f"Proxy invalid: {proxy} | Error: {e}")
            return None
    ...
    async def _filter_valid_proxies(cls, proxies: List[str]) -> List[str]:
        ...
        logger.info(f"{len(valid_proxies)} proxies are valid")
    ...
    async def get_proxies_from_config(cls, config_data: dict, validate: bool = True) -> List[str]:
        ...
        if not providers:
            logger.warning("No proxy providers initialized from config")
            return []

        logger.info("Fetching proxies from all providers...")
        fetch_tasks = [provider.get_proxies() for provider in providers]
        list_of_proxy_lists = await asyncio.gather(*fetch_tasks)

        logger.info("Closing all provider clients...")
        ...
        logger.info(f"Fetched total {len(all_proxies)} proxies")
